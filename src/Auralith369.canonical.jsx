...
export default function Auralith369(){
  ...
  const loadProject=async(file)=>{try{const project=JSON.parse(await file.text());if(project.kind!==AURALITH_PROJECT_KIND)throw new Error("Not an Auralith369 project");const data={},masks={};for(const [id,url] of Object.entries(project.layerData||{}))data[id]=await canvasFromURL(url);for(const [id,url] of Object.entries(project.maskData||{}))masks[id]=await canvasFromURL(url);ld.current=data;mks.current=masks;origS.current=project.orig?await canvasFromURL(project.orig):null;setProjectName(project.name||file.name.replace(/\.auralith$/i,""));setSz(project.size||{w:1024,h:680});setLayers(project.layers||[{id:1,n:"Background",vis:1,op:1,bl:"normal",mask:0,biLo:0,biHi:255,fx:{...dfx}}]);setAL(project.activeLayer||project.layers?.[0]?.id||1);nid.current=project.nextId||Math.max(2,...(project.layers||[]).map(l=>l.id+1));sAdj(project.adjustments||{br:100,ct:100,st:100,hu:0,bl:0,temp:0});sOvl(project.overlay?.id||"none");sOOp(project.overlay?.opacity??.5);setGuides(project.guides||[]);setSnapOn(project.snap?.enabled??1);setSnapTol(project.snap?.tolerance??9);setCaptionTone(project.captionTone||"mythic");setFg(project.colors?.fg||"#00d4aa");setBgC(project.colors?.bg||"#0a0e1a");sTxt(project.text?.txt||"PHI369");sTF(project.text?.tF||"'IBM Plex Mono',monospace");sTSz(project.text?.tSz||36);setTxtFx(project.text?.txtFx||{stroke:0,strokeW:2,strokeC:"#ffffff",shadow:0,shadowB:4,gradient:0});setSavedBatches(project.batches||[]);setVersions(project.versions||[]);setQuickA(project.quickActions||[]);setDomColors(project.dominantColors||[]);setHasI(1);sHist([]);sHP(-1);sHNm([]);setTimeout(()=>{save("Open Project");extractDominant();},80);flash("Project loaded");}catch(e){console.error(e);flash("Project load failed");}};
