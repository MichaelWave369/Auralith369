...
export default function Auralith369(){
  ...
  const exportManifest=async()=>{const payload={projectName,version:APP_VERSION,size:sz,createdAt:new Date().toISOString(),constants:{PHI,LAM,Cstar:.809017,OmegaC:.376},layers:layers.map(l=>({id:l.id,name:l.n,kind:l.kind||"pixel",visible:!!l.vis,opacity:l.op,blend:l.bl,adjustment:l.adj||null})),adjustments:adj,overlay:{id:ovl,opacity:oOp},guides,snap:{enabled:!!snapOn,tolerance:snapTol},dominantColors:domColors,gpuLab:{enabled:!!gpuEnabled,bypassed:!!gpuBypass,activePresetId:activeGpuPresetId,activePresetName:gpuPresetName,presetDirty:!!gpuPresetDirty,settings:normalizeGpuLabSettings(gpuSettings)},caption:makeCaption(),historyTail:hNm.slice(-18)};payload.manifestId=await hashText(JSON.stringify(payload));const md=[`# ${projectName||"Auralith369 Manifest"}`,"",`- Version: ${APP_VERSION}`,`- Manifest ID: ${payload.manifestId}`,`- Canvas: ${sz.w}×${sz.h}`,`- Constants: Φ=${PHI}, λ=${LAM}, C*=0.809017, Ω_c=0.376`,`- Layers: ${layers.length}`,`- Overlay: ${ovl} @ ${Math.round(oOp*100)}%`,`- Snap: ${snapOn?"on":"off"} / ${snapTol}px`,`- GPU Lab: ${gpuEnabled?"on":"off"}${gpuEnabled?` · ${gpuStatus.active?"three-webgl2":"fallback"}`:""} · cartridge=${gpuPresetName}${gpuPresetDirty?" (modified)":""}${gpuBypass?" · bypassed":""}`,"","## Caption","",payload.caption,"","## Layers",...payload.layers.map(l=>`- ${l.kind}: ${l.name} · visible=${l.visible} · opacity=${Math.round(l.opacity*100)}% · blend=${l.blend}`),"","## Palette",domColors.join(" · ")||"Not extracted yet"].join("\n");setLastManifest(payload.manifestId);downloadBlob(`${projectName||"auralith369"}.auralith-manifest.md`,"text/markdown",md);flash("Manifest exported");};
      ...
      <div style={{width:280,background:C.pn,borderLeft:`1px solid ${C.bd}`,display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflow:"auto",padding:4}}>
          ...
          {rT==="gpu"&&<>
            ...
            <div data-testid="gpu-feedback-chamber">
