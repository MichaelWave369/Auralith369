...
import GpuLabPreview from "./gpu/GpuLabPreview.jsx";
...
export default function Auralith369(){
  ...
  const[gpuEnabled,setGpuEnabled]=useState(0);
  ...
  const newC=()=>{cancelPendingHistory();clearRecoverySnapshot().catch(error=>console.warn("[Auralith369] Recovery clear for new canvas failed",error));setRecoveryCandidate(null);setRecoverySavedAt("");ld.current={};mks.current={};origS.current=null;setLayers([{id:1,n:"Background",vis:1,op:1,bl:"normal",mask:0,biLo:0,biHi:255,fx:{...dfx}}]);setAL(1);nid.current=2;setHasI(0);sOvl("none");sAdj({br:100,ct:100,st:100,hu:0,bl:0,temp:0});setSz({w:1024,h:680});sHist([]);sHP(-1);sHNm([]);histRef.current=[];hPRef.current=-1;hNmRef.current=[];sSel(null);sCrR(null);sPenP([]);setSplitV(0);setCRot(0);setGuides([]);setDomColors([]);setProjectName("auralith-project");setPosterTitle("PHI369");setPosterSub("Sovereign image alchemy");setVersions([]);setSavedBatches([]);setBatchActions([]);setBatchRec(0);setLastReceipt(null);setLastManifest("");setQuickA([]);setCaptionTone("mythic");setGpuEnabled(0);setGpuSettings(normalizeGpuLabSettings(GPU_LAB_DEFAULTS));setTimeout(()=>save("New Canvas"),0);};
  ...
  const exportGpuFrame=()=>{const dataUrl=gpuPreviewRef.current?.captureDataURL("image/png");if(!dataUrl){flash("GPU preview unavailable");return;}const a=document.createElement("a");a.download=`${projectName||"auralith369"}.gpu.png`;a.href=dataUrl;a.click();flash("GPU frame exported");};
  ...
  const exportManifest=async()=>{const payload={projectName,version:APP_VERSION,size:sz,createdAt:new Date().toISOString(),constants:{PHI,LAM,Cstar:.809017,OmegaC:.376},layers:layers.map(l=>({id:l.id,name:l.n,kind:l.kind||"pixel",visible:!!l.vis,opacity:l.op,blend:l.bl,adjustment:l.adj||null})),adjustments:adj,overlay:{id:ovl,opacity:oOp},guides,snap:{enabled:!!snapOn,tolerance:snapTol},dominantColors:domColors,gpuLab:{enabled:!!gpuEnabled,settings:normalizeGpuLabSettings(gpuSettings)},caption:makeCaption(),historyTail:hNm.slice(-18)};payload.manifestId=await hashText(JSON.stringify(payload));const md=[`# ${projectName||"Auralith369 Manifest"}`,"",`- Version: ${APP_VERSION}`,`- Manifest ID: ${payload.manifestId}`,`- Canvas: ${sz.w}×${sz.h}`,`- Constants: Φ=${PHI}, λ=${LAM}, C*=0.809017, Ω_c=0.376`,`- Layers: ${layers.length}`,`- Overlay: ${ovl} @ ${Math.round(oOp*100)}%`,`- Snap: ${snapOn?"on":"off"} / ${snapTol}px`,`- GPU Lab: ${gpuEnabled?"on":"off"}${gpuEnabled?` · ${gpuStatus.active?"three-webgl2":"fallback"}`:""}`,"","## Caption","",payload.caption,"","## Layers",...payload.layers.map(l=>`- ${l.kind}: ${l.name} · visible=${l.visible} · opacity=${Math.round(l.opacity*100)}% · blend=${l.blend}`),"","## Palette",domColors.join(" · ")||"Not extracted yet"].join("\n");setLastManifest(payload.manifestId);downloadBlob(`${projectName||"auralith369"}.auralith-manifest.md`,"text/markdown",md);flash("Manifest exported");};

  useEffect(()=>{let active=true;loadRecoverySnapshot().then(candidate=>{if(active&&candidate)setRecoveryCandidate(candidate);}).catch(error=>console.warn("[Auralith369] Recovery lookup unavailable",error));return()=>{active=false;};},[]);
  ...
  return(<div className="auralith-editor-root" style={{width:"100%",height:"100%",display:"flex",flexDirection:"column",background:C.bg,color:C.tx,fontFamily:FN,fontSize:11,lineHeight:1.35,overflow:"hidden",userSelect:"none",zoom:1.35}}
    ...
    <div style={{display:"flex",flex:1,overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",overflow:"auto",background:`radial-gradient(ellipse at center,${C.srf}28 0%,${C.bg} 100%)`,position:"relative"}} onWheel={onWheel}>
          {Boolean(drOv)&&<div style={{position:"absolute",inset:0,background:`${C.ac}08`,border:`2px dashed ${C.ac}`,borderRadius:5,display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,pointerEvents:"none"}}><span style={{fontSize:12,color:C.ac}}>Drop</span></div>}
      ...
      <div style={{width:280,background:C.pn,borderLeft:`1px solid ${C.bd}`,display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden"}}>
        <div style={{display:"flex",borderBottom:`1px solid ${C.bd}`}}>
          {["layers","fx","lut","style","forge","adj","gpu","plug","act","exp"].map(t=>(<button key={t} onClick={()=>sRT(t)} style={{flex:1,padding:"6px 0",background:"transparent",border:"none",borderBottom:rT===t?`2px solid ${C.ac}`:"2px solid transparent",color:rT===t?C.ac:C.tm,fontSize:10,cursor:"pointer",fontWeight:600,fontFamily:FN,textTransform:"uppercase"}}>{t}</button>))}
        </div>
        <div style={{flex:1,overflow:"auto",padding:4}}>
          ...
          {rT==="gpu"&&<>
            ...
            <div style={{display:"flex",gap:2,marginTop:4}}><Bt onClick={()=>setGpuSettings(normalizeGpuLabSettings(GPU_LAB_DEFAULTS))} sm style={{flex:1}}>Reset</Bt><Bt onClick={exportGpuFrame} sm style={{flex:1,color:gpuStatus.active?C.ac:C.tm}}>Export GPU PNG</Bt></div>
            <div style={{marginTop:5,padding:4,background:C.srf,borderRadius:3,border:`1px solid ${C.bd}`,fontSize:5.5,color:C.tm,lineHeight:1.5}}>Non-destructive preview. Canvas 2D remains the authority for layers, masks, undo, recovery, receipts, and standard exports.</div>
          </>}
          ...
          {rT==="act"&&<>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:0,maxHeight:200,overflow:"auto"}}>
              {hNm.map((nm,i)=>(<div key={i} style={{padding:"1px 3px",background:i===hP?C.sa:C.srf,borderRadius:1,border:i===hP?`1px solid ${C.ac}22`:"1px solid transparent",fontSize:6,color:i===hP?C.ac:C.td,cursor:"pointer"}} onClick={()=>{const s=hist[i];if(!s)return;cancelPendingHistory();restoreHistorySnapshot(s);hPRef.current=i;sHP(i);flash("History: "+nm);}}><span style={{opacity:.3,marginRight:2}}>{i+1}</span>{nm}</div>))}
            </div>
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"1px 5px",borderTop:`1px solid ${C.bd}`,background:C.pn,fontSize:5.5,color:C.tm,flexShrink:0}}>
      ...
      <span data-testid="recovery-state">Auralith369 {APP_VERSION} ∴ PHI369 Labs · Snap:{snapOn?"on":"off"} · GPU:{gpuEnabled?(gpuStatus.active?"on":"fallback"):"off"}{recoverySavedAt?" · Recovery saved":""}</span>
    </div>
  </div>);
}
