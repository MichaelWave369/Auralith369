...
export async function verifyCreativeBridgeV1(payload, { requireContentHash = true } = {}) {
  assertCreativeBridgeV1(payload);
  const contentHash = payload.contentHash ?? null;
  if (!contentHash) {
    if (requireContentHash) throw new TypeError('Creative bridge requires contentHash');
    return true;
  }
  if (!/^sha256:[0-9a-fA-F]{64}$/.test(contentHash)) {
    throw new TypeError('Creative bridge contentHash must be sha256:<64 hex>');
  }
  const actual = await sha256ImageDataUri(payload.image);
  const expected = contentHash.slice('sha256:'.length).toLowerCase();
  if (actual !== expected) {
    throw new TypeError('Creative bridge contentHash does not match image bytes');
  }
  return true;
}
...
function assertCreativeBridgeV1(payload) {
  ...
  if (!String(payload.image || '').startsWith('data:image/')) {
    throw new TypeError('Creative bridge requires image data');
  }
}
